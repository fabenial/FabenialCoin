...
static const char USAGE[] =
    R"(fabeniald )" bytecoin_VERSION_STRING R"(.
...
int main(int argc, const char *argv[]) try {
	...
	if( !export_blocks.empty() && !backup_blockchain.empty() ){
		std::cout << "You can either export blocks or backup blockchain on one run of fabeniald" << std::endl;
	...
	if (!export_blocks.empty()) {
		...
		BlockChainState block_chain_read_only(log_console, config, currency, false);
	...
	platform::ExclusiveLock coin_lock(coin_folder, "fabeniald.lock");

	logging::LoggerManager log_manager;
	log_manager.configure_default(config.get_data_folder("logs"), "fabeniald-");
...
//	block_chain.test_undo_everything(1531961);
	...
	std::cout << "fabeniald started seconds=" << double(idea_ms.count()) / 1000 << std::endl;
	while (!io.stopped()) {
	...
	std::cout << "fabeniald already running - " << ex.what() << std::endl;
